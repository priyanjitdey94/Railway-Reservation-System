...
<%!String d_time,a_time;
   int day,day1,day2,distance,distance1,distance2;
   String c1;%>

<%
int p=Integer.parseInt(request.getParameter("pnrno"));
PreparedStatement check=con.prepareStatement("select * from book where pnrno=?");
check.setInt(1,p);
ResultSet c=check.executeQuery();
if(c.next())
{
	c1=Integer.toString(c.getInt(1));
}
if(c1==null)
{%>
<tr><td><font size="5" color="#309">Invalid PNR number.Go back and enter again</font></td></tr>
<%	
}
else
{
PreparedStatement ps1=con.prepareStatement("select d_time,day,distance from book b,schedule s where b.source=s.dstation_name and b.tid=s.train_id and b.pnrno=?");
ps1.setInt(1,p);
...
if(rs1.next())
{
	  ...
	  day1=rs1.getInt(2);
	  distance1=rs1.getInt(3);
}

PreparedStatement ps2=con.prepareStatement("select a_time,day,distance from book b,schedule s where b.destination=s.dstation_name and b.tid=s.train_id and b.pnrno=?");
ps2.setInt(1,p);
...
{
%>
<tr><td><table align="center" width=750  id="tab" cellpadding="5">
  <tr>
    <td rowspan="2" width=70 ><img src="new ilogo crop.jpg" width=90 height=80></td>
    <td align="center" rowspan="2" colspan="2"><font size=6 color="#309"><b>TICKET</b></font></td>
    <td><font size=3 color="#309"><b>PNR NO.:</b></font><%=rs.getInt(1)%></td>
  </tr>
  <tr>
    <td><font size=3 color="#309"><b>Class: </b></font><%=rs.getString(7)%></td>
  </tr>
  <tr>
    <td><font size=3 color="#309"><b>Train Id: </b></font><%=rs.getInt(2)%></td>
    <td><font size=3 color="#309"><b>Train Name: </b></font><%=rs.getString(3)%></td>
    
    <td><font size=3 color="#309"><b>Date of Departure:</b></font><%=rs.getString(6)%></td>
    
  </tr>
  <tr>
    <td><font size=3 color="#309"><b>Source:</b></font><%=rs.getString(4)%></td>
    <td><font size=3 color="#309"><b>Destination: </b></font><%=rs.getString(5)%></td>
  </tr>
  <tr>
    <td><font size=5 color="#309"><b><u>Passenger Details: </u></b></font></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><%=rs.getString(15)%>&nbsp;<%=rs.getString(16)%></td>
    <td>&nbsp;</td>
    <td><font size=3 color="#309"><b>Fare: </b></font><%=rs.getString(17)%></td>
  </tr>
  <tr>
    <td align="center" colspan=4><font size=5 color="#309">Have a Safe Journey</font></td>
  </tr>
</table></td></tr>
<%}
%>
